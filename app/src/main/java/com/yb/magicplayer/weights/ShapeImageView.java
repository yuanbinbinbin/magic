...
/**
 * ShapeImageView可自由定制ImageView满足日常需求<br>
 * 矩形:正方形 width = height ,长方形 width > height <br>
 * 圆形:siv_oval = true <br>
 * 带边框：边宽度：siv_border_width 边颜色：siv_border_color<br>
 * 边角圆滑：左上角:siv_left_top_corner_radius,右上角：siv_right_top_corner_radius,左下角:siv_left_bottom_corner_radius,有下角:siv_right_bottom_corner_radius<br>
 */
public class ShapeImageView extends ImageView {
    ...
    public ShapeImageView(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs,
                R.styleable.ShapeImageView, defStyle, 0);

        final int index = a.getInt(R.styleable.ShapeImageView_android_scaleType, -1);
        Log.e("test", TAG + "index: " + getScaleType());
        if (index >= 0) {
            setScaleType(sScaleTypeArray[index]);
        }

        mLeftTopCornerRadius = a.getDimensionPixelSize(
                R.styleable.ShapeImageView_siv_left_top_corner_radius, 0);
        mRightTopCornerRadius = a.getDimensionPixelSize(
                R.styleable.ShapeImageView_siv_right_top_corner_radius, 0);
        mLeftBottomCornerRadius = a.getDimensionPixelSize(
                R.styleable.ShapeImageView_siv_left_bottom_corner_radius, 0);
        mRightBottomCornerRadius = a.getDimensionPixelSize(
                R.styleable.ShapeImageView_siv_right_bottom_corner_radius, 0);
        ...
        mBorderWidth = a.getDimensionPixelSize(
                R.styleable.ShapeImageView_siv_border_width, 0);
        if (mBorderWidth < 0) {
            throw new IllegalArgumentException("border width cannot be negative.");
        }

        mBorderColor = a
                .getColorStateList(R.styleable.ShapeImageView_siv_border_color);
        if (mBorderColor == null) {
            mBorderColor = ColorStateList.valueOf(DEFAULT_BORDER_COLOR);
        }

        isOval = a.getBoolean(R.styleable.ShapeImageView_siv_oval, false);
        a.recycle();
    ...
    public void setImageDrawable(Drawable drawable) {
        mResource = 0;
        mDrawable = ShapeRoundedCornerDrawable.fromDrawable(drawable, getResources());
    ...
    public void setImageBitmap(Bitmap bm) {
        mResource = 0;
        mDrawable = ShapeRoundedCornerDrawable.fromBitmap(bm, getResources());
        super.setImageDrawable(mDrawable);
        updateDrawable();
    }
    ...
    private Drawable resolveResource() {
        ...
        return ShapeRoundedCornerDrawable.fromDrawable(d, getResources());
    }
    ...
    private void updateDrawable() {
        ...
        ((ShapeRoundedCornerDrawable) mDrawable).setScaleType(mScaleType);
        ((ShapeRoundedCornerDrawable) mDrawable).setCornerRadii(mRadii);
        ((ShapeRoundedCornerDrawable) mDrawable).setBorderWidth(mBorderWidth);
        ((ShapeRoundedCornerDrawable) mDrawable).setBorderColor(mBorderColor);
        ((ShapeRoundedCornerDrawable) mDrawable).setOval(isOval);
    }
    ...
    static class ShapeRoundedCornerDrawable extends Drawable {

        private static final String TAG = "SRCornerDrawable";
        private static final int DEFAULT_BORDER_COLOR = Color.BLACK;
        ...
        public ShapeRoundedCornerDrawable(Bitmap bitmap, Resources r) {
            mBitmap = bitmap;
        ...
        public static ShapeRoundedCornerDrawable fromBitmap(Bitmap bitmap, Resources r) {
            if (bitmap != null) {
                return new ShapeRoundedCornerDrawable(bitmap, r);
            } else {
                return null;
            }
        }
        ...
        public static Drawable fromDrawable(Drawable drawable, Resources r) {
            if (drawable != null) {
                if (drawable instanceof ShapeRoundedCornerDrawable) {
                    return drawable;
                ...
                if (bm != null) {
                    return new ShapeRoundedCornerDrawable(bm, r);
                } else {
